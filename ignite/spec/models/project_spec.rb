...
describe Project, :type => :model do
  ...
  it 'is not valid with a description of less than 200 characters' do
    project = Project.new(name: 'Campaign', description: 'Short description', goal: '100', expiration_date: '30 days from now')
    expect(project).to have(1).error_on(:description)
    expect(project).not_to be_valid
  end

  it 'is not valid unless it has an unique name' do
    Project.create(name: 'Campaign', description: regular_description, goal: '100', expiration_date: '30 days from now')
  ...
  it 'is not valid without a goal' do
    project = Project.new(name: 'Campaign', description: regular_description, expiration_date: '30 days from now')
